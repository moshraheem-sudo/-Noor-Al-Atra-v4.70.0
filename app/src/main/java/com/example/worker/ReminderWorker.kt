...
class ReminderWorker(appContext: Context, workerParams: WorkerParameters) : Worker(appContext, workerParams) {
    override fun doWork(): Result {
        val notificationsEnabled = com.example.data.local.NotificationSettingsManager.areNotificationsEnabled(applicationContext) &&
                com.example.data.local.NotificationSettingsManager.isAzkarEnabled(applicationContext)
    ...
    private fun showNotification(context: Context, id: Int, title: String, message: String) {
        if (!com.example.data.local.NotificationSettingsManager.areNotificationsEnabled(context) ||
            !com.example.data.local.NotificationSettingsManager.isAzkarEnabled(context)
        ) {
            return
        }
