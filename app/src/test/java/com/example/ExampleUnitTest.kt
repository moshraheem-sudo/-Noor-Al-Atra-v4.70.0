  ...
  fun testQuranDatabaseDiagnosis() {
    ...
    try {
      val db = androidx.room.Room.inMemoryDatabaseBuilder(
        context,
        com.example.data.local.QuranDatabase::class.java
      ).allowMainThreadQueries().build()
      println("2. Room Database object created successfully in-memory.")
      
      val surahsList = kotlinx.coroutines.runBlocking { db.quranDao().getAllSurahsDirect() }
      surahCount = surahsList.size
      println("3. surahs table query executed, count: $surahCount")
      copySuccess = true
      db.close()
    } catch (e: Exception) {
      copySuccess = false
      copyError = e.message ?: e.toString()
      println("2/3. Failed during database query: ${e.message}")
      e.printStackTrace()
    }
