...
class PrayerAlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == AdhanAudioService.ACTION_STOP_ADHAN) {
        ...
        try {
            ...
            val isMasterEnabled = com.example.data.local.NotificationSettingsManager.areNotificationsEnabled(context)
            val isPrayerNotifEnabled = com.example.data.local.NotificationSettingsManager.isPrayerNotificationsEnabled(context)
            if (!isMasterEnabled || !isPrayerNotifEnabled) {
                return
            }
