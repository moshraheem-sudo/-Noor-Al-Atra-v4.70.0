...
object AppUpdateManager {
    ...
    const val PRAYER_TAG_API = "https://api.github.com/repos/moshraheem-sudo/Prayer-Times/releases/tags/1.40.0"
    const val PRAYER_RELEASE_PAGE = "https://github.com/moshraheem-sudo/Prayer-Times/releases/tag/1.40.0"
    const val PRAYER_DIRECT_APK = "https://github.com/moshraheem-sudo/Prayer-Times/releases/download/1.40.0/app-release.apk"
    ...
    suspend fun downloadPrayerApk(
        ...
        // 1. Try resolving through GitHub Releases API (checkPrayerUpdate)
        try {
        ...
        try {
            val listRequest = Request.Builder()
                .url("https://api.github.com/repos/moshraheem-sudo/Prayer-Times/releases")
                .header("User-Agent", "SalatiPrayerApp/${com.example.BuildConfig.VERSION_NAME}")
        ...
        // 3. Fallback direct URLs for Prayer-Times
        ...
        candidates.add("https://github.com/moshraheem-sudo/Prayer-Times/releases/download/1.40.0/app-release.apk")
        ...
        for (url in candidates) {
            ...
            downloadApkFile(
                context = context,
                downloadUrl = url,
                fileName = "Salati_PrayerTimes_Latest.apk",
                onProgress = { state ->
                    when (state) {
    ...
    suspend fun checkPrayerUpdate(): Result<AppReleaseInfo> = withContext(Dispatchers.IO) {
        try {
            ...
            // 2. Try tag endpoint if latest fails
            if (latestJson == null) {
                try {
                    val tagReq = Request.Builder()
                        ...
                        .header("Accept", "application/vnd.github.v3+json")
                        .build()
                    client.newCall(tagReq).execute().use { response ->
                        if (response.isSuccessful) {
                            val bodyStr = response.body?.string() ?: ""
                            if (bodyStr.isNotBlank() && bodyStr.startsWith("{")) {
                                latestJson = JSONObject(bodyStr)
            ...
            // 3. If still null, check all releases list
            if (latestJson == null) {
                try {
                    val listReq = Request.Builder()
                        .url("https://api.github.com/repos/moshraheem-sudo/Prayer-Times/releases")
            ...
            if (latestJson == null) {
                return@withContext Result.success(
                    AppReleaseInfo(
                        ...
                        tagName = PRAYER_CURRENT_VERSION,
                        releaseName = "تحديث تطبيق صلاتي - مواقيت الصلاة والأذان",
                        releaseNotes = "تطبيق صلاتي لمواقيت الصلاة الدقيقة والأذان بأصوات متعددة والتنبيهات المسبقة.",
                        downloadUrl = PRAYER_DIRECT_APK,
                        htmlUrl = PRAYER_RELEASE_PAGE,
                        isNewerAvailable = false
                    )
                )
            }
            ...
            val json = latestJson!!
            val tagName = json.optString("tag_name", PRAYER_CURRENT_VERSION)
            val releaseName = json.optString("name", "تحديث تطبيق صلاتي")
            val releaseNotes = json.optString("body", "يتضمن هذا التحديث تحسينات في مواقيت الصلاة، التنبيهات، والأداء العام.").ifBlank {
                "يتضمن هذا التحديث تحسينات في مواقيت الصلاة، التنبيهات، والأداء العام."
            }
